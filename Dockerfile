...
ARG VERSION
RUN CGO_ENABLED=0 go build -ldflags "-X github.com/potibm/billedapparat/cmd.Version=${VERSION}" -o billedapparat . 
...
# Copy backend build
COPY --from=backend-build --chown=appuser:appuser /app/backend/billedapparat ./billedapparat
COPY THIRD-PARTY-NOTICES.md /app/THIRD-PARTY-NOTICES.md

RUN chmod +x /app/billedapparat
