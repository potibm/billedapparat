...
* **Slide:** The smallest visual unit of content. Contains raw data (title, body, type). It does *not* know when or how it is displayed.
...
* **Social Text Slide:** A social media post containing only text/markdown. These are *not* rendered as standalone slides in the playlist sequence. Instead, they are rendered as a **Toast (or Overlay)** — a non-blocking, floating notification displayed on top of the currently active slide.
