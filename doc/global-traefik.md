...
└── certs/
    ├── billedapparat.test.crt
    ├── billedapparat.test.key
...
http:
  routers:
    billedapparat-vite:
      rule: "Host(`billedapparat.test`)"
      entryPoints:
        - websecure
      service: billedapparat-vite-service
      tls: {}
...
tls:
  certificates:
    - certFile: /certs/billedapparat.test.crt
      keyFile: /certs/billedapparat.test.key
```
...
## 2. Connect a Project (Example: Billedapparat)
...
_(For Windows/Linux installation instructions, visit the official [mkcert GitHub repository](https://github.com/FiloSottile/mkcert))._
...
Navigate to the `certs` directory of your Master-Traefik and generate the certificates for your project. We include a wildcard (`*.`) so that subdomains like `dex.billedapparat.test` or `minio.billedapparat.test` are automatically covered by the same certificate:

```bash
cd ~/Projects/traefik-proxy/certs

mkcert -cert-file billedapparat.test.crt -key-file billedapparat.test.key "billedapparat.test" "*.billedapparat.test"
```
...
Now you need to tell macOS to actually _use_ your new `dnsmasq` server whenever a `.test` domain is requested. macOS has a neat feature for this: custom resolvers per Top-Level-Domain (TLD).
...
From now on, **any** domain ending in `.test` (e.g., `billedapparat.test`, `dex.billedapparat.test`, `anything.test`) will automatically resolve to your Master-Traefik without you ever having to touch a config file again!

> **Fallback:** If you cannot install dnsmasq, you will need to manually add every single subdomain like `127.0.0.1 billedapparat.test dex.billedapparat.test` to your `/etc/hosts` file.
