...
export const ImagePreviewField = (props: ImagePreviewFieldProps) => {
  const { maxWidth = "160px", maxHeight = "90px", source, placeholder, ...rest } = props;
  const [open, setOpen] = useState(false);
  const record = useRecordContext(props);

  const getSourceValue = (obj: any, path: string) => {
  ...
  return (
    <>
      ...
      >
        ...
        <DialogContent sx={{ p: 1, display: "flex", justifyContent: "center", alignItems: "center", minWidth: 300 }}>
          <img
            src={imageUrl}
            alt="Preview"
            style={{ maxWidth: "100%", maxHeight: "85vh", objectFit: "contain", cursor: "pointer" }}
            onClick={(e) => {
              e.stopPropagation();
              setOpen(false);
            }}
